export const resources = {
  en: {
    translation: {
      flowCards: 'FlowCards',
      flow: 'Flow',
      noFlows: 'No flows found. Create one!',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      deleteConfirm: 'Are you sure you want to delete this flow?',
      deleteFailed: 'Failed to delete flow.',
      dbInit: 'Initialize DB',
      dbInitConfirm: 'All data will be deleted. Initialize?',
      dbInitDone: 'Database initialized.',
      dbInitFailed: 'Failed to initialize database.',
      flowNameEmpty: 'Flow name cannot be empty.',
      selected: '{{count}} selected',
      addCard: 'Add Card',
      editLink: 'Edit Link',
      goParent: 'Go to Parent',
      seeThrough: 'See Through',
      loading: 'Loading...',
      flowEditor: 'Flow Editor',
      flowNamePlaceholder: 'Enter flow name',
      error: 'Error',
      ok: 'OK',
      deleteConfirmTitle: 'Delete Confirmation',
      deleteSelectedConfirm: 'Are you sure you want to delete selected flows?',
      deleteFailedTitle: 'Delete Failed',
      dbInitDoneTitle: 'Initialization Complete',
      dbInitFailedTitle: 'Initialization Failed',
      search: 'Search',
      sortNameAsc: 'Name (A-Z)',
      sortNameDesc: 'Name (Z-A)',
      sortDateAsc: 'Date (Oldest)',
      sortDateDesc: 'Date (Newest)',
      newCard: 'New Card',
      newSection: 'New Section',
      grouped: 'Grouped',
      sizeSmall: 'Small',
      sizeMedium: 'Medium',
      sizeLarge: 'Large',
      title: 'Title',
      description: 'Description',
      selectCardColor: 'Select card color',
      selectColor: 'Select color',
      open: 'Open',
      remove: 'Remove',
      attachFile: 'Attach File',
      attachUrl: 'Attach URL',
      invalidUrl: 'Invalid URL',
      invalidUrlMessage:
        'Please enter a valid URL starting with http:// or https://.',
      attach: 'Attachment',
      file: 'File',
      url: 'URL',
      previewError: 'Preview Error',
      previewErrorMessage: 'Could not get a preview. Please check the URL.',
      export: 'Export',
      exportFlow: 'Export Flow ({{count}})',
      exportSuccessTitle: 'Export Successful',
      exportSuccessMessage:
        'The selected flows have been successfully prepared for export.',
      exportFailedTitle: 'Export Failed',
      exportFailedMessage:
        'An error occurred while preparing the flows for export.',
      exportError: 'Export Error',
      exportSingleFlowOnly: 'Please select only one flow to export.',
      exportConfirmTitle: 'Export Confirmation',
      exportOptionsMessage:
        'Choose an export format:\n\n- Share with App (.canvas): Best for opening directly in apps like Obsidian (attachments not included).\n- Save as File (.zip): Saves everything, including attachments, as a single .zip file.',
      exportOptionCanvas: 'Share with App (.canvas)',
      exportOptionZip: 'Save as File (.zip)',
      rename: 'Rename',
      select: 'Select',
      bulkAddCards: 'Bulk Add Cards',
      bulkAddPlaceholder: 'Enter one item per line.\nExample:\n・Item 1\n・Item 2\n- Item 3',
      create: 'Create',
      editCard: 'Edit Card',
      photo: 'Photo',
      camera: 'Camera',
      addCard: 'Add Card',
      qrScanHint: 'Point the camera at a QR code',
      recordAudio: 'Record Audio',
      audio: 'Audio',
      microphonePermissionDenied: 'Microphone permission denied',
      cameraPermissionDenied: 'Camera permission denied',
      recordingFailed: 'Recording failed',
      cutSelectCard: 'Tap a card to cut',
      cutMoving: 'Moving "{{name}}"',
      paste: 'Paste',
      pasteBreaksLinksTitle: 'Links will be removed',
      pasteBreaksLinksMessage: '{{count}} link(s) will be removed because they connect to cards in another section. Continue?',
      invalidUrlScheme: 'Only http(s) URLs can be opened.',
      cannotOpenUrl: 'Could not open the URL.',
      pasteIntoSelfMessage: 'You cannot paste a card inside itself or its descendants.',
      pasteIntoSelfHint: 'cannot paste here',
      duplicate: 'Duplicate',
      duplicateCard: 'Duplicate card',
      duplicateChildrenNotice: 'The card will be duplicated. Child cards (cards inside it) will not be included. Continue?',
      copySuffix: '(copy)',
      root: 'Top',
      importFlow: 'Import Flow',
      importDone: 'The flow was imported.',
      importUnsupportedFile: 'Only .zip or .canvas files are supported.',
      editFlow: 'Edit flow',
      name: 'Name',
      tags: 'Tags',
      tagsHint: 'Separate tags with spaces. "#" is added automatically.',
      color: 'Color',
      clear: 'Clear',
      all: 'All',
    },
  },
  ja: {
    translation: {
      flowCards: 'フローカード',
      flow: 'フロー',
      noFlows: 'フローがありません。新しく作成してください！',
      save: '保存',
      cancel: 'キャンセル',
      delete: '削除',
      deleteConfirm: 'このフローを削除しますか？',
      deleteFailed: 'フローの削除に失敗しました。',
      dbInit: 'DB初期化',
      dbInitConfirm: 'すべてのデータが削除されます。初期化しますか？',
      dbInitDone: 'データベースを初期化しました。',
      dbInitFailed: 'データベースの初期化に失敗しました。',
      flowNameEmpty: 'フロー名を入力してください。',
      selected: '{{count}}件選択中',
      addCard: 'カード追加',
      editLink: 'リンク編集',
      goParent: '親へ',
      seeThrough: 'シースルー',
      loading: '読み込み中...',
      flowEditor: 'フローエディター',
      flowNamePlaceholder: 'フロー名を入力してください',
      error: 'エラー',
      ok: 'OK',
      deleteConfirmTitle: '削除確認',
      deleteSelectedConfirm: '選択したフローを削除しますか？',
      deleteFailedTitle: '削除失敗',
      dbInitDoneTitle: '初期化完了',
      dbInitFailedTitle: '初期化失敗',
      search: '検索',
      sortNameAsc: '名前（昇順）',
      sortNameDesc: '名前（降順）',
      sortDateAsc: '日付（古い順）',
      sortDateDesc: '日付（新しい順）',
      newCard: '新しいカード',
      newSection: '新しいセクション',
      grouped: 'グループ化されました',
      sizeSmall: '小',
      sizeMedium: '中',
      sizeLarge: '大',
      title: 'タイトル',
      description: '説明',
      selectCardColor: 'カードの色を選択',
      selectColor: '色を選択',
      open: '開く',
      remove: '削除',
      attachFile: 'ファイルを添付',
      attachUrl: 'URLを添付',
      invalidUrl: '無効なURL',
      invalidUrlMessage:
        'http:// または https:// で始まる有効なURLを入力してください。',
      attach: '添付',
      file: 'ファイル',
      url: 'URL',
      previewError: 'プレビューエラー',
      previewErrorMessage: 'プレビューを取得できませんでした。URLを確認してください。',
      export: 'エクスポート',
      exportFlow: 'フローをエクスポート ({{count}}件)',
      exportSuccessTitle: 'エクスポート成功',
      exportSuccessMessage: '選択されたフローのエクスポート準備ができました。',
      exportFailedTitle: 'エクスポート失敗',
      exportFailedMessage: 'フローのエクスポート準備中にエラーが発生しました。',
      exportError: 'エクスポートエラー',
      exportSingleFlowOnly: 'エクスポートするフローは1つだけ選択してください。',
      exportConfirmTitle: 'エクスポートの確認',
      exportOptionsMessage:
        'エクスポート形式を選択してください:\n\n- アプリで共有 (.canvas): 添付ファイルを含めず、Obsidian等のアプリで直接開くのに適しています。\n- ファイルとして保存 (.zip): 添付ファイルを含む全てを、1つの.zipファイルとして保存します。',
      exportOptionCanvas: 'アプリで共有 (.canvas)',
      exportOptionZip: 'ファイルとして保存 (.zip)',
      rename: 'リネーム',
      select: '選択',
      bulkAddCards: 'カードを一括追加',
      bulkAddPlaceholder: '1行に1項目を入力してください。\n例:\n・項目1\n・項目2\n- 項目3',
      create: '作成',
      editCard: 'カード編集',
      photo: '写真',
      camera: 'カメラ',
      addCard: 'カードを追加',
      qrScanHint: 'QRコードにカメラを向けてください',
      recordAudio: '音声を録音',
      audio: '音声',
      microphonePermissionDenied: 'マイクの権限が拒否されました',
      cameraPermissionDenied: 'カメラの権限が拒否されました',
      recordingFailed: '録音に失敗しました',
      cutSelectCard: '切り取るカードをタップしてください',
      cutMoving: '「{{name}}」を移動中',
      paste: '貼り付け',
      pasteBreaksLinksTitle: 'リンクが削除されます',
      pasteBreaksLinksMessage: '他のセクションのカードとつながっている {{count}} 本のリンクが削除されます。続けますか？',
      invalidUrlScheme: 'http(s) 形式のURLのみ開けます。',
      cannotOpenUrl: 'URLを開けませんでした。',
      pasteIntoSelfMessage: 'カードを自分自身または子孫の中に貼り付けることはできません。',
      pasteIntoSelfHint: 'ここには貼り付けできません',
      duplicate: '複製',
      duplicateCard: 'カードを複製',
      duplicateChildrenNotice: 'カードを複製します。中にある子カードは複製されません。続けますか？',
      copySuffix: '(コピー)',
      root: 'トップ',
      importFlow: 'フローをインポート',
      importDone: 'フローをインポートしました。',
      importUnsupportedFile: '.zip または .canvas ファイルのみサポートしています。',
      editFlow: 'フローを編集',
      name: '名前',
      tags: 'タグ',
      tagsHint: 'スペースで区切って入力してください。「#」は自動で付与されます。',
      color: '色',
      clear: 'クリア',
      all: 'すべて',
    },
  },
  zh: {
    translation: {
      flowCards: '流程卡片',
      flow: '流程',
      noFlows: '没有找到流程，请新建！',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      deleteConfirm: '确定要删除此流程吗？',
      deleteFailed: '删除流程失败。',
      dbInit: '初始化数据库',
      dbInitConfirm: '所有数据将被删除，确定初始化？',
      dbInitDone: '数据库已初始化。',
      dbInitFailed: '数据库初始化失败。',
      flowNameEmpty: '请输入流程名称。',
      selected: '已选择{{count}}项',
      addCard: '添加卡片',
      editLink: '编辑链接',
      goParent: '返回上级',
      seeThrough: '透视',
      loading: '加载中...',
      flowEditor: '流程编辑器',
      flowNamePlaceholder: '请输入流程名称',
      error: '错误',
      ok: '确定',
      deleteConfirmTitle: '删除确认',
      deleteSelectedConfirm: '确定要删除选中的流程吗？',
      deleteFailedTitle: '删除失败',
      dbInitDoneTitle: '初始化完成',
      dbInitFailedTitle: '初始化失败',
      search: '搜索',
      sortNameAsc: '名称 (A-Z)',
      sortNameDesc: '名称 (Z-A)',
      sortDateAsc: '日期 (升序)',
      sortDateDesc: '日期 (降序)',
      newCard: '新卡片',
      newSection: '新分组',
      grouped: '已分组',
      sizeSmall: '小',
      sizeMedium: '中',
      sizeLarge: '大',
      title: '标题',
      description: '描述',
      selectCardColor: '选择卡片颜色',
      selectColor: '选择颜色',
      open: '打开',
      remove: '删除',
      attachFile: '附加文件',
      attachUrl: '附加网址',
      invalidUrl: '无效的URL',
      invalidUrlMessage: '请输入以 http:// 或 https:// 开头的有效URL。',
      attach: '附件',
      file: '文件',
      url: '网址',
      previewError: '预览错误',
      previewErrorMessage: '无法获取预览。请检查URL。',
      export: '导出',
      exportFlow: '导出流程 ({{count}})',
      exportSuccessTitle: '导出成功',
      exportSuccessMessage: '已成功准备导出所选流程。',
      exportFailedTitle: '导出失败',
      exportFailedMessage: '准备导出流程时发生错误。',
      exportError: '导出错误',
      exportSingleFlowOnly: '请只选择一个流程进行导出。',
      exportConfirmTitle: '导出确认',
      exportOptionsMessage:
        '请选择导出格式：\n\n- 在应用中共享 (.canvas): 最适合在Obsidian等应用中直接打开（不包括附件）。\n- 保存为文件 (.zip): 将所有内容（包括附件）保存为单个.zip文件。',
      exportOptionCanvas: '在应用中共享 (.canvas)',
      exportOptionZip: '保存为文件 (.zip)',
      rename: '改名',
      select: '选择',
      bulkAddCards: '批量添加卡片',
      bulkAddPlaceholder: '每行输入一个项目。\n例:\n・项目1\n・项目2\n- 项目3',
      create: '创建',
      editCard: '编辑卡片',
      photo: '照片',
      camera: '拍照',
      addCard: '添加卡片',
      qrScanHint: '将相机对准二维码',
      recordAudio: '录制音频',
      audio: '音频',
      microphonePermissionDenied: '麦克风权限被拒绝',
      cameraPermissionDenied: '相机权限被拒绝',
      recordingFailed: '录音失败',
      cutSelectCard: '点击要剪切的卡片',
      cutMoving: '正在移动「{{name}}」',
      paste: '粘贴',
      pasteBreaksLinksTitle: '将删除链接',
      pasteBreaksLinksMessage: '将删除 {{count}} 条连接到其他分区卡片的链接。是否继续？',
      invalidUrlScheme: '只能打开 http(s) 形式的 URL。',
      cannotOpenUrl: '无法打开该 URL。',
      pasteIntoSelfMessage: '无法将卡片粘贴到自身或其子孙中。',
      pasteIntoSelfHint: '不能在此处粘贴',
      duplicate: '复制',
      duplicateCard: '复制卡片',
      duplicateChildrenNotice: '复制此卡片。其中的子卡片不会被复制。是否继续？',
      copySuffix: '(副本)',
      root: '顶层',
      importFlow: '导入流程',
      importDone: '已导入流程。',
      importUnsupportedFile: '仅支持 .zip 或 .canvas 文件。',
      editFlow: '编辑流程',
      name: '名称',
      tags: '标签',
      tagsHint: '用空格分隔标签，「#」将自动添加。',
      color: '颜色',
      clear: '清除',
      all: '全部',
    },
  },
};
